		...
		for _, value in pairs(VALUE.List) do
			if value.AuraID then
				...
				-- --	if E.private.AuraWatch.checkSpellid then
						...
						-- end
				-- --	else
				-- --		local name, icon, count, _, duration, expires, caster = UnitBuff(value.UnitID, name)
				-- --		UpdateAuraFrame(KEY, value, name, icon, count, duration, expires, caster)
				-- --	end
				...
				-- --	if E.private.AuraWatch.checkSpellid then
						...
						-- end
				-- --	else					
				-- --		local name, icon, count, _, duration, expires, caster, _, _, spellID = UnitDebuff(value.UnitID, name)
				-- --		UpdateAuraFrame(KEY, value, name, icon, count, duration, expires, caster)
				-- --	end
				-- end
			end
