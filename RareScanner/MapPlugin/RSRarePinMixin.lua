...
function RSRarePinMixin:OnAcquired(npcID, npcInfo)
	...
	self.isNpc = npcInfo.atlasName == RareScanner.NPC_VIGNETTE
	...
	if (npcInfo.atlasName == RareScanner.NPC_VIGNETTE) then
...
local function filterItem(cell, args)
	local itemClassID, itemSubClassID = unpack(args)
	...
	if (private.db.loot.displayLootOnMap) then
		...
		if (next(itemsIDsFiltered) ~= nil) then
			...
			for i, itemID in ipairs(itemsIDsFiltered) do
				...
				tooltip:SetCellScript(line, j, "OnMouseDown", filterItem, { itemClassID, itemSubClassID })
